...
def test_graphql_scanner_registered_and_exposed() -> None:
    name = "graphql_check"

    assert _SCANNER_MODULE_MAP[name] == "vibe_iterator.scanners.graphql_check"
    assert name in _VALID_SCANNER_NAMES
    assert name in _DEFAULT_STAGES["pre-deploy"]
    assert name in _DEFAULT_STAGES["all"]
    assert name not in _DEFAULT_STAGES["post-deploy"]
    assert _SCANNER_META[name]["label"] == "GraphQL"


def test_webhook_scanner_registered_and_exposed() -> None:
    name = "webhook_check"

    assert _SCANNER_MODULE_MAP[name] == "vibe_iterator.scanners.webhook_check"
    assert name in _VALID_SCANNER_NAMES
    assert name in _DEFAULT_STAGES["pre-deploy"]
    assert name in _DEFAULT_STAGES["all"]
    assert name not in _DEFAULT_STAGES["post-deploy"]
...
def test_unsafe_payload_scanner_registered_and_exposed() -> None:
    name = "unsafe_payload_check"

    assert _SCANNER_MODULE_MAP[name] == "vibe_iterator.scanners.unsafe_payload_check"
    assert name in _VALID_SCANNER_NAMES
    assert name in _DEFAULT_STAGES["pre-deploy"]
    assert name in _DEFAULT_STAGES["all"]
    assert name not in _DEFAULT_STAGES["post-deploy"]
